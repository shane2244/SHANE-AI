...
export default function JournalPage() {
  const [entries, setEntries] = useState([]); const [title, setTitle] = useState(""); const [content, setContent] = useState(""); const [saved, setSaved] = useState(false);
  useEffect(() => { axios.get(`${API}/journals`).then(({ data }) => setEntries(data)).catch(() => {}); }, []);
  const save = async () => { if (!title.trim() || !content.trim()) return; const { data } = await axios.post(`${API}/journals`, { title, content, prompt: "What part of this moment do I want to remember?" }); setEntries((items) => [data, ...items]); setTitle(""); setContent(""); setSaved(true); setTimeout(() => setSaved(false), 1800); };
  return (
