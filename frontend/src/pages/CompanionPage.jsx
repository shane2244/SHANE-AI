...
export default function CompanionPage() {
  ...
  const [messages, setMessages] = useState([{ id: "welcome", role: "guide", text: "You don’t need to arrive with the right words. What feels most present in you right now?" }]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const send = async (text = input) => {
    if (!text.trim() || loading) return;
    setMessages((items) => [...items, createMessage("you", text)]); setInput(""); setLoading(true); setError("");
    try {
      const { data } = await axios.post(`${API}/companion`, { message: text });
      setMessages((items) => [...items, createMessage("guide", `${data.response} ${data.invitation}`)]);
    } catch {
      setError("I could not respond just now. Your words are still here—please try once more.");
